...
const defaultBattery = () => ({
  ...
  source_device_id: "",
  entities: {
    power: "", soc: "", state: "", temperature: "", grid_voltage: "", work_mode: "",
  ...
  charge_tiers: [
    { from_soc: 0, to_soc: 85, max_charge_w: null },
...
class BatteryManagerPanel extends HTMLElement {
  ...
  set hass(value) {
    ...
    } else if (this._tab === "overview") this._render();
  }
  ...
  _styles() {
    return `<style>
      ...
      * { box-sizing:border-box; }
      ...
      .grid-power-card { position:relative; display:flex; align-items:center; justify-content:center; text-align:center; gap:14px; margin-bottom:16px; border-left:5px solid var(--primary-color); }
      .grid-power-card ha-icon { position:absolute; left:22px; color:var(--primary-color); width:36px; height:36px; }
      .grid-power-card strong { display:block; font-size:26px; }
      .command-status { margin-top:14px; padding-top:12px; border-top:1px solid var(--divider-color); }
      .command-status h3 { font-size:14px; margin:0 0 8px; }
      .command-row { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:7px 9px; border-radius:8px; }
      ...
      @media(max-width:700px){ header{flex-wrap:wrap} nav{width:100%;display:flex;overflow:auto} main{padding:12px} }
    </style>`;
  }
  ...
  _overview() {
    ...
    const direction = Number.isFinite(Number(effectiveGrid))
      ? this._t(Number(effectiveGrid) >= 0 ? "overview.consumption" : "overview.injection") : "";
    const gridCard = `<section class="card grid-power-card"><ha-icon icon="mdi:transmission-tower"></ha-icon><div>
      <span>${this._t("overview.network_power")}</span><strong>${esc(effectiveGrid)} ${esc(rawGrid.unit)}</strong>
      <div class="muted">${esc(direction)}</div></div></section>`;
    return `${gridCard}<div class="grid">${this._config.batteries.map((b) => {
      ...
      const power = {
        state: b.power_inverted && Number.isFinite(numericPower) ? -numericPower : rawPower.state,
        unit: rawPower.unit,
      };
      const soc = this._state(b.entities.soc);
      const temp = this._state(b.entities.temperature);
      const pilotage = Boolean(b.enabled && b.operation_mode === "schedule");
      ...
      return `<section class="card"><div class="battery-head"><ha-icon icon="mdi:battery-medium"></ha-icon>
        <div><h2>${esc(b.name)}</h2><div class="muted">${esc(this._adapter(b.adapter))} · ${this._t(pilotage ? "overview.control_enabled" : "overview.monitoring")}</div></div></div>
        <div class="metrics"><div class="metric">${this._t("fields.soc")}<strong>${esc(soc.state)} ${esc(soc.unit)}</strong></div>
        <div class="metric">${this._t("fields.power")}<strong>${esc(power.state)} ${esc(power.unit)}</strong></div>
        <div class="metric">${this._t("fields.temperature")}<strong>${esc(temp.state)} ${esc(temp.unit)}</strong></div>
        <div class="metric">${this._t("fields.mode")}<strong>${decision.action === "default_mode" ? this._action("default_mode") : this._t(`modes.${b.operation_mode}`)}</strong></div></div>
        <div class="status">${this._t("overview.action")} : <b>${this._action(decision.action || "waiting")}</b><br><span class="muted">${this._reason(decision.reason || "")}</span>
          <span class="transmitted-command">${this._t("overview.transmitted_command")} : ${this._commandText(decision, b)}</span>
          ${decision.calculated_command_w !== undefined ? `<span class="muted">${this._t("overview.calculated_command")} : ${esc(decision.calculated_command_w)} W · ${this._t("overview.compensation")} : ${decision.compensation_w > 0 ? "+" : ""}${esc(decision.compensation_w)} W</span>` : ""}
          <span class="muted">${this._t("overview.pilotage_activation")} : ${this._t(pilotage ? "overview.enabled" : "overview.disabled")}</span>
          ${["marstek_entities","hoymiles_msa2"].includes(b.adapter) ? `<br><span class="muted">${this._t("overview.disabled_return")} : ${this._fallbackLabel(b)}</span>` : ""}
          ${decision.command_sent_at ? `<span class="muted">${this._t("overview.sent_at")} ${esc(new Date(decision.command_sent_at).toLocaleTimeString())}</span>` : ""}</div>
  ...
  _entityDiagnostic(device) {
    ...
    return `<details class="entities" data-device-id="${esc(device.device_id)}" ${isOpen ? "open" : ""}><summary>${this._t("diagnostic.summary", {total:rows.length, problems})}</summary>
  ...
  _batteryEditor() {
    ...
    return `<div class="toolbar"><label>${this._t("fields.battery")}<select id="batterySelect">${batteries.map((x,i)=>`<option value="${i}" ${i===this._selected?"selected":""}>${esc(x.name)}</option>`).join("")}</select></label>
      ...
      <section class="card"><fieldset><legend>${this._t("sections.grid")}</legend><div class="form-grid">
        ...
        <label>${this._t("fields.pilotage_activation")}<input data-pilotage type="checkbox" ${b.enabled && b.operation_mode==="schedule"?"checked":""}></label>
  ...
  _bind() {
    ...
    if (duplicate) duplicate.onclick = () => { const copy=structuredClone(this._config.batteries[this._selected]); copy.id=crypto.randomUUID(); copy.name += ` ${this._t("config.copy_suffix")}`; copy.enabled=false; this._config.batteries.push(copy); this._selected=this._config.batteries.length-1; this._render(); };
    ...
    const pilotage = this.shadowRoot.querySelector("[data-pilotage]");
    if (pilotage) pilotage.onchange = () => {
      const battery = this._config.batteries[this._selected];
      battery.enabled = pilotage.checked;
      battery.operation_mode = pilotage.checked ? "schedule" : "disabled";
    };
